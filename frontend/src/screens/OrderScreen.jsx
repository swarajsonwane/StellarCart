...
const OrderScreen = () => {
    ...
    return  isLoading ? <Loader/> : error ? <Message variant='danger'>{error.error || error?.data?.message}</Message> : (
        <>
                        ...
                        {!order.isPaid && (
                            <ListGroup.Item>
                                {loadingPay && <Loader/>}
                                 {isPending ? <Loader/> : (
                                    <div>
                                        <Button onClick={onApproveTest} style={{marginBottom: '10px'}} className='btn-block' type='button'>Pay with PayPal</Button>
                                        <div>
